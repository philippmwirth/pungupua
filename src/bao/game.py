...
from .rules import (
    namua_step, mtaji_step, compute_legal_action_mask, deactivate_nyumba_if_sowed,
)
from .sowing import sow
from .state import (
    GameState, INIT_BOARD, NUM_ACTIONS, NYUMBA_STOP, NYUMBA_CONTINUE, NYUMBA_COL,
)
...
class Game:
    ...
    def step(self, state: GameState, action: Array) -> GameState:
        ...
        def nyumba_branch(s: GameState) -> GameState:
            def stop(_):
                ...
                new_state = s._replace(
                    board=b, nyumba_active=na, stock=stk,
            ...
            def cont(_):
                ...
                new_cp = jnp.where(ny_pend, s.current_player, jnp.int32(1 - s.current_player))
                new_state = s._replace(
                    board=b, nyumba_active=na, stock=stk,
        ...
        def normal_branch(s: GameState) -> GameState:
            ...
            new_state = s._replace(
                board=b_out, stock=stk_out, current_player=new_cp,
                stage=new_stage, winner=winner,
                nyumba_active=na_out, nyumba_pending=ny_pend,
                pending_direction=pend_dir,
            )
            return _check_no_moves(new_state, s.current_player)
    ...
    def observe(self, state: GameState, color: Optional[Array] = None) -> Array:
        ...
        return make_observation(state.board, state.nyumba_active, color,
                                state.current_player)

    def legal_action_mask(self, state: GameState) -> Array:
        return compute_legal_action_mask(
            state.board, state.stock, state.stage,
            state.nyumba_active, state.nyumba_pending,
        )
